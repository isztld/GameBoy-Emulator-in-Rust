...
- `System::step` is the single-cycle entrypoint. Call order matters:
  ...
  2. `mmu.advance_dma` — decrements OAM DMA window counter (bus copy is instant; this counts the 160-cycle access restriction)
  3. `ppu.render_scanline` if `scanline_ready`
  4. `mmu.update_ly` / `update_ppu_stat` — sync PPU state to I/O
  5. Drain timer register writes (`timer_div_reset`, etc.)
  6. Check `ppu.vblank_entered` → set `frame_complete`
